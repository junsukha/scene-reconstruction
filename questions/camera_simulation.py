...
def calculate_camera_matrix(tx, ty, tz, alpha, beta, gamma, fx, fy, skew, u, v):
    ...
    intrinsic_matrix = np.array([fx, skew, 0, 0, fy, 0, 0, 0, 1]).reshape(3,3)
    rotate_x = np.array([1,0,0,0, np.cos(alpha), -np.sin(alpha), 0, np.sin(alpha), np.cos(alpha)]).reshape(3,3)
    rotate_y = np.array([np.cos(beta), 0, np.sin(beta), 0, 1, 0, -np.sin(beta), 0, np.cos(beta)]).reshape(3,3)
    rotate_z = np.array([np.cos(gamma), -np.sin(gamma), 0, np.sin(gamma), np.cos(gamma), 0, 0, 0, 1]).reshape(3,3)
    extrinsic_matrix_without_traslation = rotate_z @ rotate_y @ rotate_x
    
    extrinsic_matrix = np.concatenate((extrinsic_matrix_without_traslation, np.array([tx,ty,tz]).reshape(3,1)), axis=1)
